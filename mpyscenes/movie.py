...
class Movie:
    ...
    def render_clips(self, tstart=0, tstop=0):
        ...
        if self.bg_color is not None:
            bg = (
                ColorClip(size=self.size, color=color_to_tuple(self.bg_color))
                .set_start(tstart)
                .set_end(tstop)
            )
        ...
        for i, frame in enumerate(range(fstart, fstop)):
            ...
            if clip_count == 0:
                ...
                self.clips.append(bg)
            toolboxprint(
                "Rendered frame %3d / %3d at frame %3d | time %.3f sec (%d objects, %d active animations)"
                % (i + 1, (fstop - fstart), frame, t0, clip_count, anim_count)
            )
